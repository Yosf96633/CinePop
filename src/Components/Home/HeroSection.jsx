...
const HeroSection = ({ movie }) => {
  return (
    <div className="p-5 md:p-10 text-white relative z-10 top-8 xl:top-[5rem] max-md:top-3">
      ...
      <p className="mt-2 text-lg xl:text-2xl max-md:text-sm">{movie.overview}</p>
      <p className="mt-2 xl:text-xl">Release Date: <span className=' font-medium'>{movie.release_date}</span></p>
      <div className="flex items-center mt-2">
        ...
        />
        <span className="ml-2">{movie.vote_average}</span> {/* Show numeric rating */}
      </div>
    </div>
  );
};
